...
def evaluate_agent_by_scenario(model_path, pipes, scenarios, num_episodes_per_scenario=3, global_max_cost=2000000.0, global_max_pd=5000000.0):
    ...
    if 'hanoi' in scenarios:
        ...
        ongoing_debt_penalty_factor = 0.000001
        max_debt = 5000000.0
    else:
        initial_budget = 2000000.0
        budget_per_step = 1000000.0
        ongoing_debt_penalty_factor = 0.000001 # UPDATE THESE
        max_debt = 5000000.0 # UPDATE THESE
    ...
    eval_env = DummyVecEnv([
        lambda: WNTRGymEnv(
            ...
            ongoing_debt_penalty_factor=ongoing_debt_penalty_factor,
            max_debt=max_debt
        )
    ])
...
def generate_and_save_plots(model_path, log_path, drl_results, random_results, pipes, scenarios):
    ...
    log_path (str): Path to the training log
    drl_results (dict): Dictionary mapping scenarios to DRL agent rewards
    random_results (dict): Dictionary mapping scenarios to random policy rewards
    pipes (dict): Pipe configuration dictionary
    scenarios (list): List of scenarios used in training
    """
    ...
    heatmap_save_path = os.path.join(plots_dir, "Pipe_Diameter_Evolution", model_timestamp)
    ...
    # Plot 6: Simulation Failures and Reward against Time step
    ...
    # Plot 7: Pipe Diameter Heatmaps
    # plot_pipe_diameters_heatmap_over_time(
    #     model_path=model_path,
    #     pipes_config=pipes,  # Change 'pipes' to 'pipes_config' to match the function signature
    #     scenarios_list=scenarios,  # Change 'scenarios' to 'scenarios_list' to match the function signature
    #     target_scenario_name='anytown_sprawling_3' if 'anytown' in scenarios[0] else 'hanoi_sprawling_3',
    # )
...
def train_just_anytown():
    ...
    ongoing_debt_penalty_factor = 0.00001
    max_debt = 20000000.0
    ...
    vec_env_anytown = SubprocVecEnv([lambda: WNTRGymEnv(pipes, 
                                                      anytown_scenarios, 
                                                      current_max_cost=global_max_cost, 
                                                      current_max_pd=global_max_pd, 
                                                      initial_budget_per_step=budget_per_step,
                                                      start_of_episode_budget=initial_budget,
                                                      ongoing_debt_penalty_factor=ongoing_debt_penalty_factor,
                                                      max_debt=max_debt) 
                                        for _ in range(num_cpu)], start_method='spawn')
...
def train_just_hanoi(single_scenario = False):
    ...
    total_timesteps = 10000 # Short run through to check functionality
    all_scenarios = [
        'anytown_densifying_1', 'anytown_densifying_2', 'anytown_densifying_3', 'anytown_sprawling_1', 'anytown_sprawling_2', 'anytown_sprawling_3',
        'hanoi_densifying_1', 'hanoi_densifying_2', 'hanoi_densifying_3', 'hanoi_sprawling_1', 'hanoi_sprawling_2', 'hanoi_sprawling_3'
    ]
    ...
    # budget_penalty_type = 'multiplicative'
    # budget_penalty_factor = 0.5 # THIS MULTIPLIES THE REWARD BY THIS FACTOR IF THE BUDGET IS EXCEEDED
    ongoing_debt_penalty_factor = 0.000001  # Penalty per unit of debt, per step
    max_debt = 5000000.0 # Terminate episode if debt exceeds this value
    ...
    vec_env_hanoi = DummyVecEnv([lambda: WNTRGymEnv(
                                    pipes, 
                                    hanoi_scenarios, 
                                    current_max_cost=global_max_cost, 
                                    current_max_pd=global_max_pd, 
