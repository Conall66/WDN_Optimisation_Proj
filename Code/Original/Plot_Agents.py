        ...
        for info in self.locals.get("infos", []):
            ...
            if custom_info and 'reward' in custom_info:
                log_entry = {
                    ...
                    'cumulative_budget': custom_info.get('cumulative_budget'),
                    'debt_penalty_applied': custom_info.get('debt_penalty_applied', 0.0),
                    'base_reward': custom_info.get('base_reward'), # Log the pre-penalty reward
...
def plot_action_analysis(log_file="training_log.csv"):
    ...
    df_full = pd.read_csv(log_file_path)
    ...
    ax1.set_ylabel('Frequency / Count', color='tab:blue')

    # Plot pipe modifications (changes)
    if 'pipe_changes' in df_full.columns:
        df_pipe_changes = df_full[['timesteps', 'pipe_changes']].dropna()
        if not df_pipe_changes.empty:
            ax1.plot(df_pipe_changes['timesteps'], 
                    df_pipe_changes['pipe_changes'].rolling(window=50, center=True, min_periods=1).mean(), 
                    label='Pipe Modifications (Smoothed)', color='tab:blue')
            
            # Optionally add raw data points with low alpha
            ax1.scatter(df_pipe_changes['timesteps'], df_pipe_changes['pipe_changes'], 
                       alpha=0.2, color='tab:blue', s=10)
    
    ax1.tick_params(axis='y', labelcolor='tab:blue')

    # Rest of your function for budget plotting...
    ax3 = ax1.twinx()
...
def plot_upgrades_per_timestep(model_path: str, 
    ...
    for episode in range(num_episodes):
        ...
        current_episode_scenario = eval_env.current_scenario
        ...
        while not done:
            # Track the time step before taking action
            current_time_step = eval_env.current_time_step
            
            action, _ = agent.predict(obs, deterministic=True)
            obs, reward, terminated, truncated, info = eval_env.step(action)
            done = terminated or truncated
            
            # After a full major time step is completed, the environment will reset pipe_index to 0
            # This is when 'pipe_changes' will be in the info dictionary
            if eval_env.current_pipe_index == 0 and 'pipe_changes' in info:
                time_step_index = current_time_step  # Use the time step from before this step
                num_upgrades = info['pipe_changes']
                
                if 0 <= time_step_index < max_timesteps:
                    upgrades_data[time_step_index].append(num_upgrades)
                    print(f"    - Recorded {num_upgrades} upgrades for time step {time_step_index}")
                else:
                    print(f"Warning: time_step_index {time_step_index} out of bounds")
