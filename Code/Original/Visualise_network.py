...
def visualise_demands(wn, title, save_path = None, show = False):
    ...
    # Changed from 'magma' to ensure consistency
...
def visualise_network(wn, results, title, save_path, mode='2d', show=False, ax=None, min_pressure=0):
    ...
    if results:
        ...
        if max_deficit == 0:
            ...
            # Changed from RdYlGn_r to magma
            pressure_deficit_cmap = plt.get_cmap('magma')
        else:
        ...
        for node, deficit in pressure_deficits.items():
            ...
            if node in wn.junction_name_list:
                if max_deficit == 0:
                    # No deficits - use a consistent color from the magma colormap
                    pressure_colors[node] = pressure_deficit_cmap(0.2)  # Light end of magma
                else:
...
def visualise_pipe_diameters(wn, title, save_path=None, mode='2d', show=False, ax=None):
    ...
    # Changed from viridis to magma
    pipe_diam_norm = Normalize(vmin=min_diam_val, vmax=max_diam_val)
    pipe_diam_cmap = plt.get_cmap('magma') 
...
def plot_pipe_diameters_heatmap_over_time(
    ...
    try:
        ...
        # Changed from viridis to magma
        plt.imshow(heatmap_df, aspect='auto', cmap="magma", interpolation='nearest')
...
if __name__ == "__main__":
    ...
    # Plot pipe diameters heatmap for specific scenario
    agent_path = os.path.join(os.path.dirname(__file__), 'agents/agent1_hanoi_only_20250616_143017.zip')
    pipes_config = {
    ...
    scenario = 'hanoi_sprawling_3' # Example scenario to visualize
    ...
    plot_pipe_diameters_heatmap_over_time(
        ...
        target_scenario_name=scenario,
        save_dir=os.path.join(os.path.dirname(__file__), 'Plots/Pipe_Diameter_Evolution')
    )
