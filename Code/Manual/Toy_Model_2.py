...
class SAC:
    def __init__(
        ...
        if learn_alpha:
            ...
            self.alpha_optimiser = optim.Adam([self.log_alpha], lr=3e-4)
...
def optimised_wdn(env, cost_weights, max_nodes):
    ...
    for episode in range(max_episodes):
        ...
        visualise_network(env.graph, episode) # Visualise the graph after each step for debugging
    ...
    visualise_network(env.graph, "final") # Visualise the final graph after training
...
def main(max_nodes, cost_weights, grid_size):
    ...
    visualise_network(graph, "initial")
    
    # Pass to optimised_wdn() function to see training results
    optimised_wdn(graph, cost_weights, max_nodes)

def visualise_network(graph, episode, folder = "plots"):

    os.makedirs(folder, exist_ok=True) # Create the folder if it doesn't exist

    # Add subfolder
    folder = os.path.join(folder, "network_visualisations")
    os.makedirs(folder, exist_ok=True) # Create the folder if it doesn't exist
    ...
    plt.savefig(os.path.join(folder, f"Visualise_network_epsiode_{episode}.png")) # Save the figure with the number of nodes in the filename
    # plt.pause(5)
    plt.close()

def visualise_training(history, folder = "plots"): # Plot how reward function updates with each episode
    os.makedirs(folder, exist_ok=True) # Create the folder if it doesn't exist
    ...
    ax1.plot(np.log(history['training_time']), label='Log Training Time', color='blue')
    ax1.tick_params(axis='y', labelcolor='blue')

    # Create a second y-axis for the number of nodes
    ax2 = ax1.twinx()
    ax2.set_ylabel('Number of Nodes', color='orange')
    ax2.plot(np.log(history['node_counts']), label='Log Number of Nodes', color='orange')
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(os.path.join(folder, "Training_history.png")) # Save the figure with the number of nodes in the filename
    plt.close()
