...
class GNNFeatureExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            observations: Flattened observations from environment
            
        Returns:
            Extracted features tensor
        """
        # Note: Since we can't directly pass graph data through the standard observation,
        # we'll need to reconstruct the graph from the flattened observation
        # This is a limitation of using Stable-Baselines3 with custom graph data
        
        batch_size = observations.shape[0]
        features = []
        
        for i in range(batch_size):
            # For now, we'll use a simplified approach and process the flattened observation
            # In practice, you might need to modify the environment to pass graph data differently
            obs = observations[i]
            
            # Extract features using a simple MLP as fallback
            # This should be replaced with actual graph processing when graph data is available
            feature = self._process_flattened_observation(obs)
...
class GraphAwareWNTREnv(gym.Wrapper):
    ...
    def __init__(self, env, pipes_config):
        ...
        # Create a flexible observation space for graph data
        # This should be an observation space that can handle variable-sized graphs
        self.observation_space = spaces.Dict({
            'nodes': spaces.Box(low=-np.inf, high=np.inf, shape=(100, 10), dtype=np.float32),  # Max nodes, features per node
            'edges': spaces.Box(low=-np.inf, high=np.inf, shape=(200, 5), dtype=np.float32),   # Max edges, features per edge
            'globals': spaces.Box(low=-np.inf, high=np.inf, shape=(20,), dtype=np.float32),    # Global features
            'mask': spaces.Box(low=0, high=1, shape=(7,), dtype=np.int8)                       # Action mask
        })
...
def create_and_train_gnn_agent():
    ...
    # Import your environment (you'll need to import this from your script)
    ...
    # agent = GraphPPOAgent(env, pipes, verbose=1)
