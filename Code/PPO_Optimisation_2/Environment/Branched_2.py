...
def generated_branched_network(
    ...
    for u, v in mst.edges():
        ...
        G.add_edge(u, v, edge_id=edge_id, type = type, length=length, diameter=diameter, 
    ...
    for i in range(num_reservoirs):
        ...
        G.add_edge(reservoir, 
                   closest_node, 
                   edge_id=edge_id,
                   type = type, 
                   length=length, 
                   diameter=diameter, 
                   cost=unit_cost*length, 
...
# Generate hard coded branched network placement to ensure hydraulic feasibility - this network strictly has 10 nodes plus 1 reservoir and 1 tank
def generate_hard_branched(
        num_nodes = 10, 
        num_reservoirs = 1, 
        num_tanks = 1,
        ...
        num_pumps = 1,
        pump_type = 'POWER',
        pump_power = 100.0,  # in kW
        roughness_coefficients = [0.0001, 0.0002, 0.0003],
    ...
    # Create reservoir node at the top of the elevation map
    for i in range(num_reservoirs):
        if len(peak_data) > 0:
            # Select a random peak location
            x, y, z = random.choice(peak_data)
    ...
    # From the reservoir, create branched network of junctions
    current_node = f"Reservoir_{i}"
    for i in range(num_junctions):
        # Create a junction node a short distance away from the current node, ensuring that the new node is in the area and there is no overlap with existing nodes
        while True:
            x = G.nodes[current_node]['coordinates'][0] + random.choice([-10, 10])
            y = G.nodes[current_node]['coordinates'][1] + random.choice([-10, 10])
            if 0 <= x < area_size[0] and 0 <= y < area_size[1]:
                # Check if the new node overlaps with existing nodes
                if (x, y) not in [G.nodes[node]['coordinates'] for node in G.nodes()]:
                    break
        ...
        if i < num_commercial:
            demand = max_commercial_demand * random.uniform(1 - com_uncertainty, 1 + com_uncertainty)
            node_id = f"Commercial_Junction_{i}"
            G.add_node(node_id = node_id, elevation=z, demand=demand, type='commercial', coordinates=(x, y))

            edge_id = f"{current_node}_{f'Reservoir_{i}'}"
            type = 'pipe'
            length = np.linalg.norm(np.array(G.nodes[current_node]['coordinates']) - np.array(G.nodes[f'Reservoir_{i}']['coordinates']))
            pipe = random.choice(list(Pipes.keys()))
            pipe_data = Pipes[pipe]
            diameter = pipe_data['diameter']
            unit_cost = pipe_data['unit_cost']
            ...
            demand = max_residential_demand * random.uniform(1 - res_uncertainty, 1 + res_uncertainty)
            node_id = f"Residential_Junction_{i}"
            G.add_node(node_id = node_id, elevation=z, demand=demand, type='residential', coordinates=(x, y))
    ...
    # Add a pump from the reservoir to the first junction
    # Find the node connected to the reservoir
    # Indentify the resevroir node in G.nodes()
    reservoir_node = [node for node in G.nodes() if 'Reservoir' in node]
    # Get the first reservoir node
    first_reservoir = reservoir_node[0]
    first_junction = list(G.neighbors(f"Reservoir_{0}"))[0]
    edge_id = f"Reservoir_{0}_{first_junction}"
    type = 'pump'
    length = np.linalg.norm(np.array(G.nodes[f"Reservoir_{0}"]['coordinates']) - np.array(G.nodes[first_junction]['coordinates']))
    pipe = random.choice(list(Pipes.keys()))
    pipe_data = Pipes[pipe]
    diameter = pipe_data['diameter']
    unit_cost = pipe_data['unit_cost']
    ...
    # Remove the existing edge between the reservoir and the first junction
    G.remove_edge(first_reservoir, first_junction)

    # Add pump edge directly
    G.add_edge(first_reservoir, 
               first_junction, 
               edge_id=edge_id,
               type = type, 
               length=length, 
    ...
    # Connect tank to the final junction
    for i in range(num_tanks):
        while True:
            x = G.nodes[current_node]['coordinates'][0] + random.choice([-10, 10])
            y = G.nodes[current_node]['coordinates'][1] + random.choice([-10, 10])
        ...
        # Get elevation value from elevation map
        z = elevation_map[int(x), int(y)]
        G.add_node(f"Tank_{i}", elevation=z, init_level=initial_level, min_level=min_level, max_level = max_level, diameter=tank_diameter, type='tank', coordinates=(x, y))

        edge_id = f"{current_node}_{f'Tank_{i}'}"
        type = 'pipe'
        length = np.linalg.norm(np.array(G.nodes[current_node]['coordinates']) - np.array(G.nodes[f'Tank_{i}']['coordinates']))
        pipe = random.choice(list(Pipes.keys()))
        pipe_data = Pipes[pipe]
        diameter = pipe_data['diameter']
        unit_cost = pipe_data['unit_cost']
...
if __name__ == "__main__":
    ...
    landscape_type = 'hilly' # Flat or hilly
    elevation_range = [0, 100]
    ...
    # print(f"Nodes in the network: {G.nodes(data=True)}")
    # print(f"Edges in the network: {G.edges(data=True)}")

    # # Generate a hardcoded branched network
    # G, elevation_map = generate_hard_branched(
    #     num_nodes = 10, 
    #     num_reservoirs = 1, 
    #     num_tanks = 1,
    #     initial_level = 10.0,  # in m
    ...
    # # Visualise the hardcoded network
    # visualise_network(G, elevation_map, results=None, title='Hardcoded Branched Network')
    # # Check hydraulic feasibility
    # wn = convert_graph_to_wntr(G)
    ...
    # # Visualise the hardcoded network with hydraulic results
    # visualise_network(G, elevation_map, results=results, title='Hardcoded Branched Network with Hydraulic Results')
    ...
    visualise_network(G, elevation_map, results=results, title='Branched Network with Hydraulic Results')
