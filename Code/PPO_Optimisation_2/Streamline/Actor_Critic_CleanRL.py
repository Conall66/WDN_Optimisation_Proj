...
class Args:
    ...
    env_id: str = "PPOEnv"
    total_timesteps: int = 5000 # Just checking we run to completion
...
class RolloutBuffer:
    def __init__(self, buffer_size, observation_space, action_space, device, gamma=0.9, gae_lambda=0.95):
    ...
    def compute_returns_and_advantages(self, last_value):
        """
        Computes the returns and advantages using Generalized Advantage Estimation (GAE).
        This is a critical step for the PPO algorithm.
        """
        last_gae_lam = 0
        for t in reversed(range(self.size)):
            if t == self.size - 1:
                # Convert boolean to float before subtraction
...
if __name__ == "__main__":
    ...
    def save_model_with_timestamp(agent, run_name):
        ...
        model_path = f"runs/{run_name}/agent_{timestamp}.pt"
        torch.save(agent.state_dict(), model_path)
        
        # Also save as the default "agent.pt" for compatibility
        default_path = f"runs/{run_name}/agent.pt"
        torch.save(agent.state_dict(), default_path)
        
        print(f"Agent model saved to {model_path} and {default_path}")
        return model_path
    ...
    for global_step in range(0, args.total_timesteps, args.num_steps):
        ...
        for step in range(args.num_steps):
            ...
            # Prepare observation as a "batch of 1" for the GNNPolicy
            agent_obs = {
                "graph": [obs['graph']],
            ...
            # Add to buffer, storing the original, un-batched observation
            rb.add(
                obs, 
                action.flatten(),
        ...
        # Compute returns and advantages - THIS IS THE CRITICAL FIX
        ...
        # Use rb.values which holds the values from the rollout
        y_pred, y_true = rb.values.cpu().numpy(), rb.returns.cpu().numpy()
        var_y = np.var(y_true)
        ...
        wandb.log({
            ...
            "losses/approx_kl": (batch_old_log_probs - new_log_probs).mean().item(), # Note: this is for the last batch
    ...
    model_path = save_model_with_timestamp(agent, run_name)
